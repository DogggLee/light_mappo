...
CUDA 训练前（按需，当出现 `RuntimeError: CUDA error: CUBLAS_STATUS_INVALID_VALUE when calling` 类似报错时，可以执行）：
...
## Codex Working Rules
 - 在开始任务前重新整理所有的任务需求，追问存在遗漏或模糊未明确定义的地方，确认后再进行处理；
 - 任务执行过程中，尤其是编程过程中，不允许擅自添加任何有关默认值设置的条例，所有实现的函数功能均需要在明确提供指令的情况下执行，不允许擅自添加你认为应当包含的子函数功能；
 - 在当前任务处理完成后，主动追问后续处理计划；